...
{ \
	(r) = ((y + ColorModels::yuv_table.vtor_tab[v]) >> 16); \
	(g) = ((y + ColorModels::yuv_table.utog_tab[u] + ColorModels::yuv_table.vtog_tab[v]) >> 16); \
	(b) = ((y + ColorModels::yuv_table.utob_tab[u]) >> 16); \
	CLAMP(r, 0, 0xff); \
	CLAMP(g, 0, 0xff); \
	CLAMP(b, 0, 0xff); \
}
...
{ \
	(r) = y + ColorModels::yuv_table.vtor_float_tab[v]; \
	(g) = y + ColorModels::yuv_table.utog_float_tab[u] + ColorModels::yuv_table.vtog_float_tab[v]; \
	(b) = y + ColorModels::yuv_table.utob_float_tab[u]; \
}
...
{ \
	(r) = y + ColorModels::yuv_table.v16tor_float_tab[v]; \
	(g) = y + ColorModels::yuv_table.u16tog_float_tab[u] + ColorModels::yuv_table.v16tog_float_tab[v]; \
	(b) = y + ColorModels::yuv_table.u16tob_float_tab[u]; \
}

// y -> 24 bits   u, v-> 16 bits
#define YUV_TO_RGB16(y, u, v, r, g, b) \
{ \
	(r) = ((y + ColorModels::yuv_table.vtor_tab16[v]) >> 8); \
	(g) = ((y + ColorModels::yuv_table.utog_tab16[u] + ColorModels::yuv_table.vtog_tab16[v]) >> 8); \
	(b) = ((y + ColorModels::yuv_table.utob_tab16[u]) >> 8); \
	CLAMP(r, 0, 0xffff); \
	CLAMP(g, 0, 0xffff); \
	CLAMP(b, 0, 0xffff); \
}
...
{ \
	y = ((ColorModels::yuv_table.rtoy_tab[r] + ColorModels::yuv_table.gtoy_tab[g] + ColorModels::yuv_table.btoy_tab[b]) >> 16); \
	u = ((ColorModels::yuv_table.rtou_tab[r] + ColorModels::yuv_table.gtou_tab[g] + ColorModels::yuv_table.btou_tab[b]) >> 16); \
	v = ((ColorModels::yuv_table.rtov_tab[r] + ColorModels::yuv_table.gtov_tab[g] + ColorModels::yuv_table.btov_tab[b]) >> 16); \
	CLAMP(y, 0, 0xff); \
	CLAMP(u, 0, 0xff); \
	CLAMP(v, 0, 0xff); \
}
...
{ \
	y = ((ColorModels::yuv_table.rtoy_tab16[r] + ColorModels::yuv_table.gtoy_tab16[g] + ColorModels::yuv_table.btoy_tab16[b]) >> 8); \
	u = ((ColorModels::yuv_table.rtou_tab16[r] + ColorModels::yuv_table.gtou_tab16[g] + ColorModels::yuv_table.btou_tab16[b]) >> 8); \
	v = ((ColorModels::yuv_table.rtov_tab16[r] + ColorModels::yuv_table.gtov_tab16[g] + ColorModels::yuv_table.btov_tab16[b]) >> 8); \
	CLAMP(y, 0, 0xffff); \
	CLAMP(u, 0, 0xffff); \
	CLAMP(v, 0, 0xffff); \
}
...
static inline void transfer_RGBA8888_to_TRANSPARENCY(unsigned char *(*output),
	unsigned char *input, int (*bit_counter))
{
...
static inline void transfer_RGBA8888_to_RGB8bg(unsigned char *(*output),
	unsigned char *input, int bg_r, int bg_g, int bg_b)
{
...
static inline void transfer_RGBA8888_to_BGR565bg(unsigned char *(*output),
	unsigned char *input, int bg_r, int bg_g, int bg_b)
{
	unsigned int r, g, b, a, anti_a;
...
static inline void transfer_RGBA8888_to_RGB565bg(unsigned char *(*output),
	unsigned char *input, int bg_r, int bg_g, int bg_b)
{
	unsigned int r, g, b, a, anti_a;
...
static inline void transfer_RGBA8888_to_BGR888bg(unsigned char *(*output),
	unsigned char *input, int bg_r, int bg_g, int bg_b)
{
	unsigned int r, g, b, a, anti_a;
...
static inline void transfer_RGBA8888_to_RGB888bg(unsigned char *(*output),
	unsigned char *input, int bg_r, int bg_g, int bg_b)
{
	unsigned int r, g, b, a, anti_a;
...
static inline void transfer_RGBA8888_to_BGR8888bg(unsigned char *(*output),
	unsigned char *input, int bg_r, int bg_g, int bg_b)
{
	unsigned int r, g, b, a, anti_a;
...
static inline void transfer_YUVA16161616_to_RGB_FLOAT(float *(*output), uint16_t *input)
{
	float y;
	int u, v;
	float r, g, b, a;
