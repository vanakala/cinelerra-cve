...
ExportEDLAsset::ExportEDLAsset(EDL *edl)
{
	this->edl = edl;
...
void ExportEDLAsset::double_to_CMX3600(ptstime seconds, double frame_rate, char *str)
{
	char tmp[20];
...
void ExportEDLAsset::edit_to_timecodes(Edit *edit, char *sourceinpoint,
	char *sourceoutpoint, char *destinpoint, char *destoutpoint, char *reel_name)
{
	Asset *asset = edit->asset;
	Track *track = edit->track;
	double frame_rate = edlsession->frame_rate;

	ptstime edit_sourcestart;
	ptstime edit_sourceend;
	ptstime edit_deststart;
	ptstime edit_destend;
...
// Find the track...
	int serial = 0;
	Track *track;
	for(track = edl->first_track(); track; track = track->next)
	{
	...
	if(edl_type == EDLTYPE_CMX3600)
	{
		...
		for(Edit *edit = track->edits->first; edit; edit = edit->next)
		{
			...
			if(edit->transition &&
				!strcmp(edit->transition->plugin_server->title, "Dissolve"))
			{
				char last_sourceout[12];

				edit_to_timecodes(edit->previous, sourceinpoint,
					last_sourceout, destinpoint, destoutpoint,
					reel_name);
				edit_to_timecodes(edit, sourceinpoint, sourceoutpoint,
					destinpoint, destoutpoint, reel_name);

				if(last_dissolve)
				{
					fprintf(fh, "%03d %8s %s %4s %3s", colnum,
						reel_name, avselect, edittype, cutinfo);
			...
			else
			{
				edit_to_timecodes(edit, sourceinpoint, sourceoutpoint,
					destinpoint, destoutpoint, reel_name);
				fprintf(fh, "%03d %8s %s %4s %3s", colnum, reel_name,
					avselect, edittype, cutinfo);
				fprintf(fh, " %s %s", sourceinpoint, sourceoutpoint);
				fprintf(fh, " %s %s", destinpoint, destoutpoint);
				fprintf(fh,"\n");
				last_dissolve = 0;
...
{
	mwindow_global->defaults->get("EDLEXPORT_PATH", path);
	mwindow_global->defaults->get("EDLEXPORT_TYPE", edl_type);
	mwindow_global->defaults->get("EDLEXPORT_TRACKNUMBER", track_number);
}

void ExportEDLAsset::save_defaults()
{
	mwindow_global->defaults->update("EDLEXPORT_PATH", path);
	mwindow_global->defaults->update("EDLEXPORT_TYPE", edl_type);
	mwindow_global->defaults->update("EDLEXPORT_TRACKNUMBER", track_number);
}
...
void ExportEDL::run()
{
	int result = 0;
	int root_w, root_h;
	exportasset = new ExportEDLAsset(master_edl);
...
// Get format from user
	...
	// FIX
		...
		if(!result)
		{
			...
			// Fix "EDL"!
			exportedl_window->path_recent->add_item("EDLPATH",
				exportasset->path);
		}
...
ExportEDLWindow::ExportEDLWindow(int x, int y, ExportEDL *exportedl,
	...
	1)
{
	...
	x += 300;
	path_recent = new BC_RecentList("EDLPATH", mwindow_global->defaults,
