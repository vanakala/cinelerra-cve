	...
	int options)
{
	...
	if(options & SUPPORTS_VIDEO)
	{
		...
		EXRConfigVideo *window = new EXRConfigVideo(parent_window, asset, cx, cy);
...
size_t FileEXR::get_memory_usage()
{
	...
	if(temp_y) result += (size_t)asset->width * asset->height * 3 / 2;
	return result;
}
...
int FileEXR::write_frame(VFrame *frame, VFrame *data, FrameWriterUnit *unit)
{
	...
	int components = ColorModels::components(native_cmodel);
