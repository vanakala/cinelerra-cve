...
int YUVStream::open_read(const char *path) 
{
	...
	if (stream_fd < 0) 
	{
		errormsg("Failed to open \"%s\" for reading. \n%m\n", path);
		return 1;
	}
...
int YUVStream::read_frame_raw(uint8_t *data, int frame_size) 
{
...
int YUVStream::write_frame_raw(uint8_t *data, int frame_size)
{
#if (MJPEGTOOLS_Y4M_WRITE_FRAME_HEADER__3ARGS)
...
int YUVStream::seek_frame(framenum frame_number)
{
	...
	if (position == 0) 
	{
		...
		errormsg("seek_frame(%d): position was zero\n", frame_number);
	}
...
double YUVStream::get_aspect_ratio()
{
	...
	mpeg_aspect_code_t code = 
		mpeg_guess_mpeg_aspect_code(2, sar, 
						get_width(),
						get_height());
	y4m_ratio_t aspect_ratio =  mpeg_aspect_ratio(2, code);
