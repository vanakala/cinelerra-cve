...
void MainProgressBar::update_length(double length)
{
	this->length = length;
...
// Print ETA on title
	...
	if(current_eta - last_eta > 1000)
	{
		...
		if(value > 0.001)
			eta = current_eta / 1000 * length /
				value - current_eta / 1000;
		else
			eta = 0;

		Units::totext(time_string, eta, TIME_HMS2);

		update_current_title(_("%s ETA: %s"), default_title, time_string);
...
void MainProgressBar::get_time(char *text)
{
	...
	Units::totext(text, current_time, TIME_HMS2);
}
...
MainProgressBar* MainProgress::start_progress(const char *text, ptstime total_length,
	int use_window)
{
	int cx, cy;
