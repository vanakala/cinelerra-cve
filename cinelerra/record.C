...
framenum Record::batch_video_offset()
{
	return (framenum)((double)get_current_batch()->file_offset * 
		default_asset->frame_rate + 0.5);
}

samplenum Record::current_audio_position()
{
	if(file)
	{
/* FIXIT
...
framenum Record::current_video_position()
{
	if(file)
	{
/* FIXIT
...
samplenum Record::current_duration_samples()
{
	return (samplenum)((float)batches.values[current_batch]->duration * default_asset->sample_rate + 0.5);
}

framenum Record::current_duration_frames()
{
	return (framenum)((float)batches.values[current_batch]->duration * default_asset->frame_rate + 0.5);
}
...
samplenum Record::sync_position()
{
...
int Record::set_loop_duration(int value)
{
	loop_duration = value; 
	return 0;
}
...
int Record::get_playback_buffer() { return mwindow->edl->session->playback_buffer; }
