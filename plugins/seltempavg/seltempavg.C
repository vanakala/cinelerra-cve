...
void SelTempAvgMain::update_gui()
{
	if(thread) 
	{
		if(load_configuration())
		{
			...
			thread->window->offset_fixed_value->update((long long int)config.offset_fixed_value);
		...
		thread->window->offset_restartmarker_pos->update((long long int)restartoffset);
