...
SNDFILE*
sf_open_fd	(FILE *fileptr, char *path, int mode, SF_INFO *sfinfo, int close_desc)
{	SF_PRIVATE 	*psf ;
	int			error ;
	...
	psf->filedes = fileno(fileptr) ;
	psf->fileptr = fileptr;
	psf->fileoffset = psf_ftell (psf) ;
// Copy it for stat and ftruncate
	strcpy (psf->filename, path) ;
